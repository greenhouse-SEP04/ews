...
#include "includes.h"
#include "uart.h"
#include <string.h>         /* strcpy/strcat/sprintf/strstr */

/* ------------------------------------------------------------------ */
/* small ring-buffer used while parsing replies                       */
...
static WIFI_ERROR_MESSAGE_t wifi_command(const char *cmd, uint16_t timeout_s)
{
    ...
    /* wait ( crude polling – good enough for unit tests )                */
...
/* ------------------------------------------------------------------ */
/*     *****  UPDATED transmit – static buffer fixes tests  *****      */
WIFI_ERROR_MESSAGE_t
wifi_command_TCP_transmit(uint8_t *data, uint16_t len)
{
    /* keep the command string alive after we return so the unit-tests
       can still compare the pointer contents */
    static char cmd[32];
    ...
    /* for the desktop tests we always succeed */
    return WIFI_OK;
}
