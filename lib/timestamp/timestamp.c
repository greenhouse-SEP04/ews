...
/* -------------------------------------------------------------------------- */
static void http_response_callback(void)
{
    resp_index = strlen(resp_buf);           /* not used elsewhere, but kept */
}
...
bool timestamp_sync_via_http(void)
{
    ...
    if (wifi_command_create_TCP_connection(
            "94.130.142.35", 80,
            http_response_callback, recv_buf) != WIFI_OK)
    {
    ...
    const char http_request[] =
        "GET /v1/forecast?latitude=55.67&longitude=12.56&current_weather=true "
        "HTTP/1.1\r\n"
    ...
    if (wifi_command_TCP_transmit((uint8_t *)http_request,
                                  strlen(http_request)) != WIFI_OK)
    {
        uart_send_string_blocking(USART_0,
                                  "Error request HTTP\n");
        return false;
